...
class ShowData(wx.Dialog):
    def __init__(self, parent, sensor):
        ...
        if test_toggle == "true":
            list_of_files = glob.glob(root + 'data/tests/' + self.sensor + '*.dat')
        else:
            list_of_files = glob.glob(root + 'data/*.dat')
        ...
        # If there are no files found
            ...
            self.content = ["(There are no data files yet)\n"]
    ...
    def InitUI(self):
        ...
        if self.sensor == "BMP/BME":
            ...
            self.content = ["(There is no data for that sensor yet)\n"]
        if start or end:
